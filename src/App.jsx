...
// Add these states to UnifiedMapsAssistant
...
// Add this function for natural conversation
const conversationalResponse = (type, details) => {
  switch(type) {
    case 'initial_planning':
      return `I'll help you get to ${details.destination}. First, are you hoping to arrive by a specific time?`;
    
    case 'weather_check':
      return `I notice it's ${getCurrentWeather()}. Would you prefer a route with good visibility and well-lit roads?`;
    
    case 'break_suggestion':
      return `Since this is a ${details.duration} minute trip, would you like me to plan any breaks along the way? I can help find good stops for rest, food, or fuel.`;
    
    case 'route_confirmation':
      return `I've found a route that matches your preferences. It ${
        drivingPreferences.avoidHighways ? 'avoids highways and ' : ''
      }should take about ${details.duration} minutes. Would you like to hear about any potential stops along the way?`;
...
// Enhance the processVoiceCommand function
const processVoiceCommand = (command) => {
  const lowerCommand = command.toLowerCase();
  
  // New navigation command handling with conversation
  if (lowerCommand.includes('navigate to') || lowerCommand.includes('take me to')) {
    ...
    setResponse(conversationalResponse('initial_planning', { destination }));
    return;
  }

  // Handle time-related responses
  if (conversationContext.isPlanning && lowerCommand.includes('by') && lowerCommand.includes('arrive')) {
    ...
    if (timeMatch) {
      ...
      setResponse(conversationalResponse('weather_check', {}));
    }
    return;
  }

  // Handle preference responses
  if (lowerCommand.includes('yes') || lowerCommand.includes('yeah')) {
    if (!conversationContext.hasAskedPreferences) {
      setDrivingPreferences(prev => ({
        ...prev,
        preferWellLit: true
      }));
      const duration = Math.floor(Math.random() * 60) + 30; // Simulated duration
      setConversationContext(prev => ({
        ...prev,
        hasAskedPreferences: true,
        currentTripDuration: duration
      }));
      setResponse(conversationalResponse('break_suggestion', { duration }));
      return;
    }
  }

  // Handle break preferences
  if (lowerCommand.includes('break') || lowerCommand.includes('stop')) {
    const plan = createTimeBudgetPlan(conversationContext.currentTripDuration);
    setCurrentPlan(plan);
    setResponse(conversationalResponse('route_confirmation', {
  ...
  // (keep existing command processing for other features)
};
  const planDay = () => {
    try {
